...
basis-set dependence \citep{adch}. \revised{Multiwfn applies the same atomic-dipole
correction to the Becke populations, so the released \texttt{becke} scheme is
ADC-corrected Becke rather than raw Becke populations; ADC-Becke and ADCH share the
correction but not the underlying partition. Because ORCA and Multiwfn are independent
codes, comparisons between the ORCA-derived (Mulliken, L\"owdin) and Multiwfn-derived
schemes (App.~\ref{app:charge_correlations}) are cross-code as well as cross-scheme.}
...
Each split folder contains \revised{nine} LMDB files keyed on a single string identifier
...
\item \texttt{charge.lmdb}: per-atom charges nested by scheme (\texttt{hirshfeld}, \texttt{cm5}, \texttt{adch}, \texttt{becke} (ADC-corrected), \texttt{mulliken\_orca}, \texttt{loewdin\_orca}); \texttt{hirshfeld}/\texttt{cm5} also carry a total dipole, \texttt{adch}/\texttt{becke} carry total and per-atom dipoles.
...
\item \revised{\texttt{energy.lmdb}: OMol25 reference labels aligned to the same key space --- total energy (eV), per-atom forces (eV/\AA), and the source OMol25 record identifier.}
\end{itemize}
...
\revised{\textbf{Cross-package reproducibility.} On 87 wavefunctions drawn from seven
verticals (organics, transition-metal complexes, solvation shells, noble-gas systems), we
recomputed charges with HORTON~\citep{horton} and QTAIM topology with
Critic2~\citep{critic2} from the same wavefunctions (App.~\ref{app:xval}). Hirshfeld
charges agree to a median of 0.004~$e$ ($r=0.9993$, 1{,}030 atoms); raw Becke populations
agree to 0.005~$e$ ($r=0.994$) once both codes use the same atomic-radii convention. On
the 1{,}439 bond critical points found by both codes, $\rho$, $\nabla^2\rho$ and the
kinetic- and energy-density fields agree with $r\geq0.998$ and median differences below
$10^{-6}$~a.u., and the BCP sets, which define the QTAIM graph edges, match exactly in 77
of 87 structures. The two codes do sometimes differ, but only in structures with ECPs and
mostly at weak ($\rho<0.05$~a.u.) contacts; on the 62 all-electron structures these fields
agree with $r=1.0000$. This tests the software axis at a fixed
wavefunction; sensitivity to level of theory is a separate axis, studied for QTAIM
features by~\citet{D5DD00220F}.}
