...
\begin{figure}[t]
  ...
  \caption{Overview of the \texttt{generator} post-processing pipeline. Starting from an
    ...
    heterographs via our sister package \texttt{qtaim\_embed}\protect\footnote{\protect\url{https://anonymous.4open.science/r/qtaim_embed-0E05}}.}
  \label{fig:pipeline}
\end{figure}
...
\revised{A single folder runs with one command (App.~\ref{app:single_folder}).}
...
We exercised the pipeline across three HPC schedulers \ifanon(PBS Pro, Slurm, and Flux, at
three computing centers whose names are withheld for review)\else(PBS Pro on ALCF Polaris,
Slurm on NERSC Perlmutter, Flux on LLNL Tuolumne)\fi. PBS and Slurm use a controller-and-workers Parsl
