...
def test_get_qtaim_data():
    ...
    for keys in qtaim_dict.keys():
        if "atom" in keys:
            ...
            atom_count += 1
            assert (
                len(qtaim_dict[keys]) == reference_atom
            ), f"atom keys are not the same length"
        if "bond" in keys:
            if bond_count == 0:
                reference_bond = len(qtaim_dict[keys])
            assert (
                len(qtaim_dict[keys]) == reference_bond
            ), f"bond keys are not the same length"
            bond_count += 1
...
def test_get_data():
    data_full = get_data(root=root_folder, full_descriptors=True)
    ...
    assert (
        len(data_full["extra_feat_bond_esp_total"][2]) == 20
    ), "extra_feat_bond_esp_total is not the correct length"
    assert np.isclose(
        data_full["extra_feat_bond_esp_total"][2][-2], 0.8080011181, atol=1e-5
    ), "extra_feat_bond_esp_total is not correct"
    assert len(data_full["ibsi_data"][2]) == 28, "ibsi_data is not the correct length"
    assert np.isclose(
        data_full["ibsi_data"][2][(5, 17)], 0.05525, atol=1e-5
    ), "ibsi_data is not correct"
    assert (
        len(data_full["bond_list_fuzzy"][2]) == 32
    ), "bond_list_fuzzy is not the correct length"
    assert (0, 8) in data_full["bond_list_qtaim"][2], "bond_list_qtaim val error"
    assert (
        len(data_full["bond_list_qtaim"][2]) == 20
    ), "bond_list_qtaim is not the correct length"
    assert (
        len(data_full["mbis_charge"][2]) == 18
    ), "mbis_charge is not the correct length"
    assert np.isclose(
        data_full["mbis_charge"][2][-2], 0.32306682, atol=1e-5
    ), "mbis_charge is not correct"
    assert len(data_full["bader_spin"][2]) == 18, "bader_spin is not the correct length"
    assert np.isclose(
        data_full["bader_spin"][2][-2], 0.0, atol=1e-5
    ), "bader_spin is not correct"
    assert np.isclose(
        data_full["hirshfeld_dipole_mag"][2], 0.960007, atol=1e-5
    ), "hirshfeld_dipole_mag is not correct"
    assert (
        len(data_full["becke_atomic_dipole"][2]) == 18
    ), "becke_atomic_dipole is not the correct length"
    ref_dipole = np.array([0.486347, 0.067275, 0.246652])
...
def test_gather_impute():
    ...
    impute_result = gather_impute(root=root_folder, full_descriptors=True)
...
def test_parse_bond():
    ...
    assert len(bond_res["bond_list_ibsi"]) > len(
        bond_res_10["bond_list_ibsi"]
    ), "ibsi data is not being filtered correctly"
...
def test_get_qtaim_data_impute():
    ...
    # print(qtaim_dict.keys())
    keys = get_keys_qtaim(qtaim_dict)
    qtaim_impute = get_qtaim_data_impute(qtaim_dict, keys)
    ...
    for keys in keys:
        if "atom" in keys:
            ...
            atom_count += 1
            assert (
                len(qtaim_impute[keys]) == reference_atom
            ), f"atom keys are not the same length"
        if "bond" in keys:
            if bond_count == 0:
                reference_bond = len(qtaim_impute[keys])
            assert (
                len(qtaim_impute[keys]) == reference_bond
            ), f"bond keys are not the same length"
            bond_count += 1
...
def test_output():
    ...
    vdd_dipole_ref = [
        [0.128154, -0.461206, 0.90739],
        [-0.008995, 1.689744, 0.29848],
        [-0.084769, -0.967985, 0.019081],
        [0.064753, 0.341573, 0.905806],
    ]
    adch_atom_dipole_ref = [
        [0.042746, -0.048822, 0.136883],
        [-0.000284, -0.156197, -0.102598],
        [-0.080911, 0.011131, 0.093894],
        [0.011559, -0.063826, 0.193715],
    ]
