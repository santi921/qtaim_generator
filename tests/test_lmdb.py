...
class TestLMDB: 
    ...
    json_2_lmdbs(dir_active, dir_active, "charge", "merged_charge.lmdb", chunk_size, clean=True)
    json_2_lmdbs(dir_active, dir_active, "bond", "merged_bond.lmdb", chunk_size, clean=True)
    json_2_lmdbs(dir_active, dir_active, "other", "merged_other.lmdb", chunk_size, clean=True)
    json_2_lmdbs(dir_active, dir_active, "qtaim", "merged_qtaim.lmdb", chunk_size, clean=True)
    
    inp_files_2_lmdbs(dir_active, dir_active, "merged_geom.lmdb", chunk_size, clean=True)
    ...
    def test_write_read(self): 
        ...
        assert dict_orca_5['1']['density_alpha'] == orca5_qtaim_json["1"]["density_alpha"], f"Expected {dict_orca_5['1']['density_alpha']}, got {orca5_qtaim_json['1']['density_alpha']}"
    ...
    def test_merge(self):
        ...
        for lmdb_file in [self.charge_lmdb, self.bond_lmdb, self.other_lmdb, self.qtaim_lmdb, self.geom_lmdb]:
            env = lmdb.open(
                lmdb_file,
                subdir=False,
                readonly=True,
