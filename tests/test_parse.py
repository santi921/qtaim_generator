...
def get_full_qtaim():
    ...
    qtaim_file = str(TEST_FILES / "test_prop_full.txt")
...
def get_critical_point_atom():
    ...
    cp_file = str(TEST_FILES / "test_atom_cp.txt")
...
def get_critical_point_bond():
    ...
    cp_file = str(TEST_FILES / "test_bond_cp.txt")
    with open(cp_file, "r") as f:
        cp = f.readlines()
    return cp
...
def get_critical_point_ring():
    ...
    cp_file = str(TEST_FILES / "test_ring_cp.txt")
    with open(cp_file, "r") as f:
        cp = f.readlines()
    return cp
...
def test_parse_full_cp():
    dict_qtaim = get_qtaim_descs(str(TEST_FILES / "test_prop_full.txt"))
    # print(dict_qtaim)
    count_atoms = 0
    count_bonds = 0
...
def test_parse_dft_inp():
    dict_dft = dft_inp_to_dict(str(TEST_FILES / "test_dft.in"))
...
def test_parse_dft_inp_orca():
    dict_dft = orca_inp_to_dict(str(TEST_FILES / "orca" / "orca.inp"))
    ...
    dict_dft = orca_inp_to_dict(str(TEST_FILES / "orca" / "orca5.inp"))
...
def test_only_atom_cps():
    qtaim_dict = get_qtaim_descs(str(TEST_FILES / "CPprop.txt"))
...
def test_find_cp_map():
    qtaim_dict = get_qtaim_descs(str(TEST_FILES / "CPprop_w_bond_paths.txt"))
    atom_dict, atom_dict_bonds = only_atom_cps(qtaim_dict)
    dict_dft = dft_inp_to_dict(str(TEST_FILES / "input_bond_paths.in"))
...
def test_merge_qtaim_inds():
    ...
    dict_qtaim = get_qtaim_descs(str(TEST_FILES / "CPprop_bond_map.txt"))
    cp_dict = merge_qtaim_inds(
        qtaim_descs=dict_qtaim,
        bond_list=bonds,
        define_bonds="distance",
        dft_inp_file=str(TEST_FILES / "input_bond_map.in"),
        inp_type="xyz",
    )
...
def test_bond_cp_via_qtaim_bond_defns():
    qtaim_dict = get_qtaim_descs(str(TEST_FILES / "CPprop_w_bond_paths.txt"))
    dft_dict = dft_inp_to_dict(str(TEST_FILES / "input_bond_paths.in"))
